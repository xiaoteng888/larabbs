...
trait ActiveUserHelper{
    ...
    protected $pass_days = 30; //多少天发表过内容
    protected $user_number = 6; //取出来多少用户
    // 缓存相关配置
    protected $cache_key = 'smbbs_active_users';
    protected $cache_seconds = 65 * 60;

    public function getActiveUsers()
    {
    	// 尝试从缓存中取出 cache_key 对应的数据。如果能取到，便直接返回数据。
        // 否则运行匿名函数中的代码来取出活跃用户数据，返回的同时做了缓存。
        return Cache::remember($this->cache_key,$this->cache_seconds,function(){
    ...
    {
    	Cache::put($this->cache_key,$active_users,$this->cache_seconds);
    }
}